...
- **가상 회로 교환(Virtual circuit switching) 통신**
  ...
  - TCP가 가상 회로 교환 방식이라 생각할 수 있지만, **TCP의 Packet들은 동일한 경로로 전송되지 않음**  
  ( 어떻게 보면 **TCP 자체가 가상 회로 역할**을 한다고 볼 수 있음 )
...
- 데이터 무결성을 위한 체크섬, Datagram의 소스 및 대상에서 다양한 기능을 처리하기 위해 **포트 번호를 제공**

- **패킷 손실, 순서 없음, 중복 전송 등의 문제**가 있어 오류 검사 및 수정이 필요하지 않거나  
시간이 생명인 **실시간 애플리케이션에서 수행되는 용도에 적합함**
