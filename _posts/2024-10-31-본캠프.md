...
- 그 때 팀원분들 중 한 분이 정리된 디렉토리 구조와 스타일을 정리해서 오셨다!  
(만세! 빨리 뜯어서 적용 시켜야지)
...
- 어제는 GitHub Page 로 빌드하는 것에 고집해서 Jekyll로 이어지게 하려고 했는데.. <mark>그냥 index.html에 합쳐져 나오게 만들면 되는 것 이었다..</mark>

---

### 멤버 소개페이지 프로젝트 방명록 삭제 기능 구현 +@

- 삭제 기능 그냥 구조로 들어가 remove()만 하면 되는거 아니야?  
=> 라는 생각을 했던 때가 있엇죠..

- 사실 **페이지만** 본다면 틀린말은 아니지만.. 페이지에 댓글 데이터를 저장하기위해 API연동을 시켰기에..  
=> 댓글들을 안보이게해도 API 서버에 남아있어 다시 불러와 진다는 것이다!
...
=> 그럼 doc id를 지정할 수 있게 adddoc을 setdoc으로 바꾸자!  
...
- 그 때 한 분이 작성했던 "시간"과 "이름"으로 삭제할 doc를 구별하는건 어떠냐고 조언을 해주셨다..
...
docs.forEach((docRef) => {
    ...
    li.append(div);
    li.append(btn_del);
    //Comment는 붙일 곳을 querySelector로 불러왔다.
    Comment.prepend(li);
});
...
=> 관리자 모드가 되면 댓글 삭제는 그냥 가능해야되는거 아냐?(비밀번호 건너뛰기)  

>> **전개**

- 세션에서 값을 읽어오는 구문을 복사해서 분석했다.
