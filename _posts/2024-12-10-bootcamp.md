...
1. 3-way-handshake
    ...
    1. Client에서 Server에 연결 요청을 하기위해 SYN 패킷을 보내며 SYN_SENT 상태가 된다.

    2. Server는 SYN 패킷을 받고 SYN_RCV(Received)로 상태 변경!

        => 그리고 요청을 정상적으로 받았다는 대답(ACK)와 Client도 포트를 열어달라는 SYN 을 같이 보낸다.

    3. Client에서는 SYN+ACK 를 받고 ESTABLISHED로 상태를 변경하고 Server에 요청을 잘 받았다는 ACK 를 전송!

        => ACK를 받은 Server는 상태가 ESTABLSHED로 변경되며 연결이 정상적으로 이어졌다!
