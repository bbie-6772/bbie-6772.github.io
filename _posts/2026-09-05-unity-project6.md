...
**필수 컴포넌트를 강제하는 표시를 자식 오브젝트에 붙였다.** 이 표시는 **같은 오브젝트**에 그 컴포넌트를 강제하는 것이라, 자식에 경험치 저장소가 하나 더 생겨버렸다. **경험치가 두 곳으로 갈라진다.** 화면에는 아무 에러도 안 났다.

**비교하려던 자리에 대입을 썼다.** `==`를 써야 하는데 `=`를 하나만 썼다. 다행히 여기서는 컴파일러가 막아줬다. 담긴 게 참·거짓이 아니라서다.
