...
1. map의 비동기 콜백함수(async function)로 Promise 자체가 반환되는데..

2. 보통의 Promise 들은 변수에 대입을 하는 과정에서 실행이 되는데 콜백함수의 경우에 실행이 되지 않은채로 반환되는 것 같다.

3. 비동기 콜백함수의 return 값은 Promise가 실행되었을 때 반환이 되기에 Promise를 실행시켜(fulfill) 줘야 한다!
