...
그리고 만든 *Repository*에 기초적인 *index.html*을 넣고 분업화를 위해 *_include*폴더를 만들어  
각자의 나눈 역할에 맞게 파일을 만들어두었다.
...
(사실 처음엔 경로오류로 "./"가 상대경로 라는 사실을 알게됬지만..)  
...
> 그렇게 파일들이 제대로 통합되어 로컬 실행을 했을 땐 잘 됬기에.. ~~문제 없겠지?~~
...
아무리 값을 ~~.text() => DOMParser().parseFromString(x, 'text/html')~~  등등으로  변환을 시도해 보았으나..
  
> 화면에 나오는 값은 오로지 <span>**[Object HTMLDocument]**</span>
  
그렇게 포기하려던 찰나.. 이 블로그와 똑같이 Jekyll 로 빌드가 되면 include를 쓸 수 있지 않을까?! 해서 사용 해봤지만 어림도 없었다..
  
왜 어림도 없엇는지를 알게된건 파일을 Jekyll 빌드에 인식시키기 위해 위에 "---" 를 2줄 써줘야 됬단 사실..  
(3시간 뻘짓한 후 / 참고로 폴더이름도 '_include'가 아닌 '_includes'로 해야한다..)  
  
그렇게 무사히 수정을 마친 후 팀원분들께 설명하여 **dev branch(Main branch직전)**에  **Pull request**를 마쳤다!
